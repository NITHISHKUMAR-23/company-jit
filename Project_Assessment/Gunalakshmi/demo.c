...
int main() {
    ...
    WAVHeader header = readWAV("C:\\Users\\JIIN18540724\\Downloads\\audiopassthrough\\mcpcm_48k_5.1.wav", &leftChannel, &rightChannel, &centerChannel, &lfeChannel, &leftSurroundChannel, &rightSurroundChannel);
    ...
    // Add LFE to low-frequency components (left, right, center, surround)
    for (int i = 0; i < header.dataSize / sizeof(int16_t) / 6; i++) {
        // Low-frequency addition
        leftChannel[i] += filteredLFE[i];
        rightChannel[i] += filteredLFE[i];
        centerChannel[i] += filteredLFE[i];
        leftSurroundChannel[i] += filteredLFE[i];
        rightSurroundChannel[i] += filteredLFE[i];
    ...
    writeWAV("C:\\Users\\JIIN18540724\\Downloads\\audiopassthrough\\Output_with_LFE_10dB.wav", header, interleavedOutput, header.dataSize / sizeof(int16_t));
