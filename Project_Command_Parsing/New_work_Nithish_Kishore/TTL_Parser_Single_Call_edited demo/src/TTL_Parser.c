...
char *c_to_python_call(char *function_name, char *command, int parameter, char *dictionary, char *path, uint32_t reference_file_type, uint32_t *Parameter_buffer)
{
    ...
    if (pModule != NULL)
    {
        ...
        if (pFunc && PyCallable_Check(pFunc))
        {
            ...
            PyObject *pArg = PyTuple_Pack(6,
                                          ...
                                          PyLong_FromLong(*Parameter_buffer));

// Nithish Modifications
/*
            // // Prepare Parameter_buffer list
            // PyObject *paramBufferList = PyList_New(0);
            // // Loop through the Parameter_buffer and add the elements to the list
            // for (uint32_t i = 0; Parameter_buffer[i] != 0; i++) // Assuming Parameter_buffer is null-terminated or has some sentinel value
            // {
            //     paramBufferList[i] = PyLong_FromLong(Parameter_buffer[i]);
            // }
            ...
            // Update the tuple argument by adding the Parameter_buffer list
...
int command_identification(char *command, char *path, int file_format, uint32_t *Parameter_buffer, char **Command_Name) // Function to identify and validate a command based on the file format and command string
{
    ...
    *Command_Name = c_to_python_call("command_validation", command, false, "false", path, file_format, Parameter_buffer);
        ...
        if (line != NULL && line[0] != '\n')
        {
            ...
            if (hexValue != NULL && hexValue[0] != '\0')
            {
                ...
                if (Status == SUCCESS)
                {
                    ...
                    if (command_identification(Full_command, Reference_File_Path, reference_file_type, &Command_Name, Parameter_buffer) == 0)
                    {
