...
char *c_to_python_call(char *function_name, char *command, int parameter, int parameter_no, char *dictionary, char *path, uint32_t reference_file_type)
{
...
char *extractHexValues(const char *line)
{
    ...
    if (tokenSendln != NULL)
    {
        ...
        if ((hexValue[MAX_HEX_SIZE - 1] == '\'') || (hexValue[MAX_HEX_SIZE - 2] == '\0') || (tokenSendln[HEX_WITH_SENDLN] != '\''))
        {
...
/*  Function to print a long horizontal line for visual separation in the output file
    This function prints a line of hyphens to the provided output file, creating a visual separator.
...
/* Function to print a short horizontal line for visual separation in the output file
   This function prints a line of hyphens to the provided output file, creating a visual separator.
*/
    ...
    while (fgets(line, sizeof(line), input_file))
    {
        ...
        if (hexValue != NULL && hexValue[0] != '\0')
        {
            ...
            if (Status == SUCCESS)
            {
                ...
                //     {
                //         // Determine the parameter number (1-based index)
