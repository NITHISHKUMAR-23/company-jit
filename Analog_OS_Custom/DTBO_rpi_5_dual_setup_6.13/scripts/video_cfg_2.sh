...
# Reset media topology
media-ctl -d "$MEDIA_DEV" -r

# Set v4l2 capture parameters
...
media-ctl -d "$MEDIA_DEV" --set-route "'csi2' [0/0 -> 1/0 [0x01], 0/1 -> 2/0 [0x01]]"

# ---------- SET FORMATS FOR csi2 PADS ----------
...
# ---------- ENABLE LINKS TO CFE ----------
sudo media-ctl -d "$MEDIA_DEV" -l "'csi2':0 -> 'rp1-cfe-csi2-ch0':0 [1]"
sudo media-ctl -d "$MEDIA_DEV" -l "'csi2':1 -> 'rp1-cfe-csi2-ch1':0 [1]"
sudo media-ctl -d "$MEDIA_DEV" -l "'csi2':2 -> 'rp1-cfe-csi2-ch2':0 [1]"
