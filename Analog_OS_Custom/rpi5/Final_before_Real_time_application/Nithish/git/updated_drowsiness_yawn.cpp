...
const double MAR_THRESH = 0.65;        // follow python threshold
const double EAR_CLOSED_TIME = 1.2;    // seconds for drowsiness confirmation
const double MAR_YAWN_TIME  = 0.3;    // reduce for instant detection

// ---- Calibration & smoothing ----
const double EAR_INIT_FACTOR = 0.72;
const double MAR_INIT_FACTOR = 0.58;

const double EAR_SMOOTH_ALPHA = 0.10; // slightly faster update
const double MAR_SMOOTH_ALPHA = 0.25; // faster MAR response

const double MIN_EAR = 0.14;
const double MIN_MAR = 0.22;

const int CALIB_FRAMES = 30;
const int MEDIAN_WIN = 3; // faster response
...
// New: mouth_aspect_ratio for 68-point LBF landmarks
double mouth_aspect_ratio(const vector<Point2f>& lm){
    if(lm.size() < 68) return 0.0;

    double v1 = norm(lm[61] - lm[67]);
    double v2 = norm(lm[62] - lm[66]);
    double v3 = norm(lm[63] - lm[65]);
    double h = norm(lm[60] - lm[64]);
    ...
    while(running){
        ...
        if(!faces.empty()){
            ...
            if(ok && !L.empty()){
                ...
                double mRaw = mouth_aspect_ratio(lm);
                if(mEMA==0) mEMA=mRaw;
                ...
                // ---------- Yawn Logic (instant update) ----------
                yawnFlag = (m > mThr);
                yawn = yawnFlag; // immediately update
