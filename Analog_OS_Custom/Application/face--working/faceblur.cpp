...
void configureCamera() {
    // Previous subdev configuration commands
    ...
    // Additional quality improvement controls
    system("v4l2-ctl -d /dev/v4l-subdev2 --set-ctrl=exposure=2000");
    system("v4l2-ctl -d /dev/v4l-subdev2 --set-ctrl=analogue_gain=100");
    system("v4l2-ctl -d /dev/v4l-subdev2 --set-ctrl=digital_gain=1900");
}

// Convert Bayer RGGB pattern to RGB
...
// Static face detector for efficiency
...
// Draw red box and apply stronger blur on detected faces
void blurAndMarkFaces(unsigned char* rgbBuffer, int width, int height, const std::vector<cv::Rect>& faces) {
    ...
    for (const auto& face : faces) {
        // Draw red rectangle before blurring
        cv::rectangle(img, face, cv::Scalar(0, 0, 255), 2); // BGR -> Red box
        // Stronger Gaussian blur for more privacy
        cv::Mat faceROI = img(face);
        cv::GaussianBlur(faceROI, faceROI, cv::Size(61, 61), 50); // Larger kernel & sigma
    }
}
