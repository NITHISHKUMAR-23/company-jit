...
void Configure_Hash_Command(ADI_PKTE_COMMAND *command1)
{
	...
	command1->aes_key_length=aes_key_length_other;
...
void Configure_Decryption_Command(ADI_PKTE_COMMAND *command0)
{
	/*Configure the mode of encryption operation in ADI_PKTE_COMMAND structure */
	command0->opcode=opcode_decrypt;
	command0->direction=dir_inbound;
#ifdef AES_Decryption
...
int main()
{
	...
	PkteList1.pDestination = (uint32_t*)&destination0;
	...
	/*Configure source, key, destination pointers in ADI_PKTE_LIST structure */
	PkteList2.pSource = (uint32_t*)&source0;
	PkteList2.pDestination = (uint32_t*)&hash_output;
	PkteList2.pIV = (uint32_t*)&IV;
	PkteList2.pKey = (uint32_t*)&Key;
	PkteList2.pIdigest = (uint32_t*)&IDigest;
	PkteList2.nUserID = 0xF;
	PkteList2.nSrcSize = sizeof(source0);
	...
	Configure_Decryption_Command(&command0);
	Configure_Hash_Command(&command1);
	Configure_Hash_Decrypt_Command(&command2);
	...
	{
		if(i< sizeof(destination0)>>2)
		{
			if(destination0[i]!=destination1[i])
				Error_Count++;
		}
		else
		{
			if(hash_output[i- (sizeof(destination0)>>2)]!=destination1[i])
				Error_Count++;
		}
	}
