...
#endif
static uint32_t g_iMSITxBuffer[ADI_CACHE_ROUND_UP_SIZE((TOTAL_LENGTH/4u), uint32_t)];
...
__attribute__ ((aligned (4096)))
#endif
static uint32_t g_iMSIRxBuffer[ADI_CACHE_ROUND_UP_SIZE((TOTAL_LENGTH/4u), uint32_t)];
...
int main(int argc, char *argv[])
{
	...
	/*Changing clock frequency to 50 MHz for data transfer operation*/
	eResult =adi_emsi_ChangeClockFreq(hDev0,DIVIDER_VALUE);
	CheckResult(eResult);

	/* Sets DMA type for an upcoming EMSI transfer(s) buffer boundary parameter set to 4 kB
	 * (i.e. DMA should see 4 kB continuous memory ).*/
	eResult = adi_emsi_SetDmaType(hDev0,ADI_EMSI_DMA_USED_SDMA,ADI_EMSI_SDMA_BUF_BDARY_4K);
	CheckResult(eResult);
	...
	/*Perform a Multi-block data write in blocking mode.*/
	eResult= adi_emsi_Blocking_Transfer(hDev0,ADI_EMSI_TRANSFER_TYPE_MULTI_BLOCK_WR_PREDEF,NO_OF_BLOCKS,START_ADDRESS_BLOCK,&g_iMSITxBuffer[0]);
	CheckResult(eResult);
	printf(" \n Write transfers Done");

	/*Perform a Multi-block data read in blocking mode.*/
	eResult= adi_emsi_Blocking_Transfer(hDev0,ADI_EMSI_TRANSFER_TYPE_MULTI_BLOCK_RD_PREDEF,NO_OF_BLOCKS,START_ADDRESS_BLOCK,&g_iMSIRxBuffer[0]);
	CheckResult(eResult);
	printf(" \n Read transfers Done");
