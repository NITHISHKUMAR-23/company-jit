...
static void VerifyCrcData (void)
{
    ...
    if(FailCount > 0)
    {
    	DEBUG_PRINT("CRC fill verification failed\n");
    }
    else
    {
    	DEBUG_PRINT("CRC fill verification passed\n");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    /* Configure CRC for Fill mode */
    eResult =  adi_crc_DmaDataFill(hCrcDev,&DestBuf[0], CRC_BUF_NUM_ELEMENTS);
    CHECK_RESULT(eResult);
    /* Verify CRC data */
