...
/* Source channel data buffer for CRC DMA transfers */
ADI_CACHE_ALIGN static uint32_t SrcBuf[ADI_CACHE_ROUND_UP_SIZE(CRC_BUF_NUM_ELEMENTS, uint32_t)];
/* Source channel data buffer for CRC DMA transfers */
ADI_CACHE_ALIGN static uint32_t DestBuf[ADI_CACHE_ROUND_UP_SIZE(CRC_BUF_NUM_ELEMENTS, uint32_t)];
...
static void VerifyCrcData (void)
{
    /* Loop variable */
    uint32_t        i;
    ...
    for (i = 0u; i < CRC_BUF_NUM_ELEMENTS; i++)
    {
        /* IF (Source buffer data not valid) */
    ...
    if(FailCount > 0)
    {
    	DEBUG_PRINT("CRC data transfer failed\n");
    }
    else
    {
    	DEBUG_PRINT("CRC data transfer finished successfully\n");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    /* Open CRC driver and initialize it with static configuration */
    ...
    /* Fill CRC buffer with predefined values*/
    FillDataBuf();

    /* Expected CRC of the buffer*/
    ExpecCrc = EXPECTED_CRC;

    /* Program CRC with expected CRC value to generate error if there is a mismath*/
    eResult=adi_crc_SetExpectedVal(hCrcDev,ExpecCrc);
    CHECK_RESULT(eResult);

    /* Enable CRC in Memory Transfer mode*/
    eResult =  adi_crc_MemXfer(hCrcDev,&SrcBuf[0],&DestBuf[0], CRC_BUF_NUM_ELEMENTS);
    CHECK_RESULT(eResult);
    ...
    /* Compare CRC result*/
    if(ReadCrc !=ExpecCrc)
    	DEBUG_PRINT("CRC computation verification failed\n");
    else
    	DEBUG_PRINT("CRC computation verification Pass\n");

    /* CRC data compare verification*/
    VerifyCrcData();
