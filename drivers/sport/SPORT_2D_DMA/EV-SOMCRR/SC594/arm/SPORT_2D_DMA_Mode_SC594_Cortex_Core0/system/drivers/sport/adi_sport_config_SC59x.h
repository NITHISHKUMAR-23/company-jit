...
#define ADI_SPORT0A_DIV_FSDIV                 511ul
 ...
 **********************************************************************************************/
/*!SPORT0B Instance*/
#define ADI_SPORT0B_INSTANCE                   1ul
 ...
 *     	for (wordlength <= 7 bits)    -----> MSIZE = 1 Byte , PSIZE = 1 Byte.
 *  	for (wordlength <= 15 bits)   -----> MSIZE = 2 Bytes , PSIZE = 2 Bytes.
 *  	for (wordlength <= 15 bits) and (DMA packing is enabled) -----> MSIZE = 2 Bytes , PSIZE = 4 Bytes.
 *  	for (wordlength <= 31 bits)   -----> MSIZE = 4 Bytes , PSIZE = 4 Bytes.
 * */
#define ADI_SPORT0B_DMA_MSIZE 		 		 ENUM_DMA_CFG_MSIZE04 /* 4  bytes transfer (32  bits) */
#define ADI_SPORT0B_DMA_PSIZE  		 		 ENUM_DMA_CFG_PSIZE04 /* 4  bytes transfer (32  bits) */
...
/*! Specifies the SPORT0B_Control register Configuration*/
#define ADI_SPORT_0B_CTL   ((ADI_SPORT0B_CTL_DTYPE<<BITP_SPORT_CTL_B_DTYPE)|(ADI_SPORT0B_CTL_LSBF<<BITP_SPORT_CTL_B_LSBF)|(ADI_SPORT0B_CTL_SLEN<<BITP_SPORT_CTL_B_SLEN)|(ADI_SPORT0B_CTL_PACK<<BITP_SPORT_CTL_B_PACK)|(ADI_SPORT0B_CTL_ICLK<<BITP_SPORT_CTL_B_ICLK)|(ADI_SPORT0B_CTL_CKRE<<BITP_SPORT_CTL_B_CKRE)|(ADI_SPORT0B_CTL_IFS<<BITP_SPORT_CTL_B_IFS)|(ADI_SPORT0B_CTL_LFS<<BITP_SPORT_CTL_B_LFS)|(ADI_SPORT0B_CTL_FSED<<BITP_SPORT_CTL_B_FSED)|(ADI_SPORT0B_CTL_SPTRAN<<BITP_SPORT_CTL_B_SPTRAN)|(ADI_SPORT0B_CTL_OPMODE<<BITP_SPORT_CTL_B_OPMODE))
...
/*! Select the number of transmit or receive clock cycles that the Half-SPORT counts before generating a frame sync pulse.
* FSDIV = (SPORT_ACLK / SPORT_AFS) - 1 */
#define ADI_SPORT0B_DIV_FSDIV                  511ul
...
/*! Multi-Channel (TDM) Mode : Selects the start location for the Half-SPORT's active window of channels within the 1024-channel range.
 *  For Right Justified Mode : The least significant 6 bits of WOFFSET serve as the delay count (DCNT) field*/
#define ADI_SPORT0B_MCTL_WOFFSET 				 0x0
/*! Selects the window size for the Half-SPORT's active window of channels.
* WSIZE = (number of channel slots) -1 */
#define ADI_SPORT0B_MCTL_WSIZE 				      0x15
/*! Selects the delay (in serial clock cycles) between the Half-SPORT's multichannel frame sync pulse and channel 0 */
#define ADI_SPORT0B_MCTL_MFD					 0x1
/*! Enables DMA data packing for transmit and enables DMA data unpacking for the Half-SPORT's multichannel data transfers.
 * Configured as:
 * 		0 : Disable
 * 		1 : Enable*/
#define ADI_SPORT0B_MCTL_MCPDE 				     0x1
/*! Enables multichannel operations for the Half-SPORT.
 *  * Configured as:
 * 		0 : Disable
 * 		1 : Enable*/
#define ADI_SPORT0B_MCTL_MCE 					 0x1
...
#define ADI_SPORT_0B_CS1						 0x0
#define ADI_SPORT_0B_CS2		        		 0x0
#define ADI_SPORT_0B_CS3						 0x0
