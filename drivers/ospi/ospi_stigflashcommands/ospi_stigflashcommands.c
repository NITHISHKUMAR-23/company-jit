...
*
* @brief     This is the primary source file for OSPI Flash command using stig mode example.
*
*/
...
int main(int argc, char *argv[])
{
	...
	/* This function runs different STIG commands in a sequence to do some meaningful operations on the Macronix flash.
	 ...
	 * - Enter OPI STR mode
	 * - Read 8 Bytes in OPI STR mode (STIG)
	 * - Exit OPI STR mode
	 * - Read 8 Bytes in single mode (STIG)
	 * */
	eResult = Ospi_Flash_command_Sequence_Macronix( *phDevice, &STIG_Command );

	if(eResult != ADI_OSPI_SUCCESS)
	{
		printf("One of the STIG command returned an error. Error code returned %d \n", eResult);
		printf("Fail \n");
		return 0;
	}
