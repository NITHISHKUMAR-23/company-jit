...
//#define ENABLE_4BYTE_ADDRESSING
...
#if CONFIG_BOOT_UART_BAUD_RATE || CONFIG_BOOT_SPI_CLOCK_RATE || CONFIG_BOOT_OSPI_CLOCK_RATE
...
/**
 * @brief       The main code for Preload code for SC59x series of processors.
 * @details     The main code for Preload code for SC59x series of processors.
...
uint32_t initcode(ADI_ROM_BOOT_CONFIG* pBootStruct)
{
	...
	*pREG_SPI2_TXCTL |= ENUM_SPI_TXCTL_TX_EN;
	*pREG_SPI2_RXCTL |= ENUM_SPI_RXCTL_RX_EN;
	*pREG_SPI2_CTL 	 |= ENUM_SPI_CTL_EN;

	*pREG_SPI2_TXCTL|= ENUM_SPI_TXCTL_TWC_EN ;
	*pREG_SPI2_TXCTL |= ENUM_SPI_TXCTL_TX_EN;
	*pREG_SPI2_CTL 	 |= ENUM_SPI_CTL_EN;
	...
	*pREG_SPI2_TWC	= 	1;
	*pREG_SPI2_TFIFO = 0xB7;
	while(!(*pREG_SPI2_STAT & BITM_SPI_STAT_TF));
	*pREG_SPI2_STAT = BITM_SPI_STAT_TF;
	...
	*pREG_SPI2_CTL 	 &= ~(ENUM_SPI_CTL_EN);
	*pREG_SPI2_TXCTL&= ~(ENUM_SPI_TXCTL_TX_EN|ENUM_SPI_TXCTL_TWC_EN);

	//Set the FLASH back to memory mapped mode with 4 byte address size
	*pREG_SPI2_MMRDH&=~BITM_SPI_MMRDH_ADRSIZE;
	*pREG_SPI2_MMRDH|=(4<<BITP_SPI_MMRDH_ADRSIZE);

	*pREG_SPI2_RXCTL=0x5;
	*pREG_SPI2_TXCTL = 0x5;
	*pREG_SPI2_CTL 	 = 0x80000043;
#endif
...
static int32_t bcmd_device(ADI_ROM_BOOT_CONFIG* pBootStruct)
{
    int32_t bcmd = pBootStruct->dBootCommand;
