...
/* FreeRTOS FPU specific definitions. */
/* configUSE_TASK_FPU_SUPPORT determines how the task context is
 * configured for FPU using the following values:
 *  1 will only configure the FPU context for tasks which call the
 *    macro portTASK_USES_FLOATING_POINT().
 *  2 Will configure the FPU context for all tasks.
 *  We default to 2 because some library routines such as memcpy make
 *  use of the FPU registers.
 */
