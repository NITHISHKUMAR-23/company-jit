...
- Resolved an issue where certain `styles` were not applied correctly.
- Updated documentation for improved clarity and usability.

## Demo App v3.0.0

- Enhanced visuals for various modes.
- Introduced a `styles` category in `config.json` for visual customization
  (refer to `config_options.md` for details).
- Enabled selection of different `config.json` files via URL.
- Updated the `test_api` Python application to parse an `interface.json` file
  instead of hardcoding arguments, and added new endpoints for proxy parameters.
- Beta feature: Proxy parameters control.

## Demo App v2.2.0

- Updated to the new product package definition.

## Demo App v2.1.1

- Updated visual assets for audio modes.
- Added demo dar files and adjusted the `config.json` files accordingly.
