...
HEADER_H = b'\xAA'
HEADER_L = b'\x55'
MSG_LOC = b'\x01\x00\x00\x00'
#MSG_LOC_SETP = b'\x02\x00\x00\x00'
HDR_ID_CONF = b'\x08\x00\x00\x00'
#SET_PARAM_ID = b'\x02\x00\x00\x00'
SET_PARAM = b'\x03'
SET_ACT_ARCHIVE = b'\x01'
SET_ARCHIVE_CMD = b'\x02'
FOOTER_H = b'\x55'
FOOTER_L = b'\xAA'
pre_qualifier = b'\x02\x00\x00\x00\x02\x00\x00\x00'
...
active_archives = {
    "": [],
    "Jasmin_IN_Demo_AIQ-H-P-U_DPL5-6-0_V02.eval.dar": [
      "%1%2_AIQ-U_FS",
      "%2%2_AIQ-U_DSU_FS",
      "%3%2_AIQ_FS",
      "%4%2_AIQ-P_FS",
...
active_configuration = "%3%2_AIQ_FS"
active_archive = "Jasmin_IN_Demo_AIQ-H-P-U_DPL5-6-0_V02.eval.dar"
...
def activeArchive():
    ...
    def error_finder(err):
        conversion_dict = {246:'Ok', 247:'General Error', 248:'Memory Error', 249:'Unsupported Block size', 250:'Unsupported stream format', 251: 'Invalid Dar', 252:'Invalid parameter ID', 253:'Invalid parameter range'}
        if err in conversion_dict:
            return conversion_dict[err]
    global active_archive, active_configuration
    parameter_length = b'\x26\x00\x00\x00'
    if request.method == 'PUT':
        ...
        conf_data = active_configuration.encode().ljust(1016, b'\x00')
        #conf_data = active_configuration.encode()
        #conf_data = b'\x25\x32\x25\x32\x5F\x41\x49\x51\x2D\x55\x5F\x44\x53\x55\x5F\x46\x53' + b'\x00' * 999
        #CONFIG = b'\x25\x32\x25\x32\x5F\x41\x49\x51\x2D\x55\x5F\x44\x53\x55\x5F\x46\x53'nO 
        #print(CONFIG)
        crc =  calculate_crc( parameter_length + MSG_LOC + HDR_ID_CONF + conf_data)
        crc = struct.pack('B', crc)
        data = HEADER_H + HEADER_L +  parameter_length + MSG_LOC + HDR_ID_CONF + conf_data + crc + FOOTER_H + FOOTER_L
        print(data)
        ser.write(data)
        try:
...
def volume():
    ...
    parameter_size = b'\x14\x00\x00\x00'
    ...
    if request.method == 'PUT':
        ...
        try:
            ...
            #jasmin integrated code
            print("Gain linear = {}".format(validated_volume))
            plain = 20*log(validated_volume,10)
            plain = round(plain)
            print("Plain = {}".format(plain))
            #normalized = plain_to_normalized(plain)      
            #print("Normalized = {}".format(normalized))
            
            #active_data = struct.pack('f', plain)
            #active_data =  b'\x19\x00\x13\x00\x01\x00\x00\x00\x00\x00\x00\x00'
            
            #active_data =  b'\x19\x00\x24\x00\x01\x00\x00\x00\x00\x00\xF0\xC2'#0dB
            active_data =  b'\x19\x00\x13\x00\x01\x00\x00\x00\x00\x00\x70\xC1'#35dB
            print('active {}'.format(active_data))
            active_data = active_data + b'\x00' * 1004
            crc = calculate_crc(  parameter_size + pre_qualifier +active_data)
            crc = struct.pack('B', crc)
            
            #active_data = active_data.ljust(1016, b'\x00')
            '''if len(active_data) != 26:
                raise ValueError("Byte representation is not 26 bytes long")'''
           
             
            # 170 85 20 0 0 0 2 0 0 0 2 0 0 0 25 0 19 0 1 0 0 0 0 0 112 193    
            data = HEADER_H + HEADER_L  + parameter_size + pre_qualifier  + active_data + crc + FOOTER_H + FOOTER_L
            #print(data)
            ser.write(data)
        except ValueError:
