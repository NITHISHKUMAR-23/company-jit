...
__attribute__((section(".l2_uncached")))
#endif
ADI_CACHE_ALIGN static int   int_SP0ABuffer1[COUNT*4];
#if defined(__ADSPCORTEXA55__)
__attribute__((section(".l2_uncached")))
#endif
ADI_CACHE_ALIGN static int   int_SP0ABuffer2[COUNT*4];
        ...
        case ADI_SPORT_EVENT_RX_BUFFER_PROCESSED:
        		...
        		if(CallbackCount==1)
        		{
        			for(i=0,j=0;i<COUNT;i+=2,j+=8)
        		   {
        			   ...
        			   int_SP0ABuffer1[j+2]=int_SP0ABuffer4[i];/*Copy ADC buffer to DAC buffer */
        			   int_SP0ABuffer1[j+3]=int_SP0ABuffer4[i+1];/*Copy ADC buffer to DAC buffer */
        			   ...
        			   int_SP0ABuffer1[j+6]=int_SP0ABuffer4[i];/*Copy ADC buffer to DAC buffer */
        			   int_SP0ABuffer1[j+7]=int_SP0ABuffer4[i+1];/*Copy ADC buffer to DAC buffer */
        		   }
        		}
        		...
        		if(CallbackCount==2)
				{
        			for(i=0,j=0;i<COUNT;i+=2,j+=8)
				   {
					   ...
					   int_SP0ABuffer2[j+2]=int_SP0ABuffer5[i];/*Copy ADC buffer to DAC buffer */
					   int_SP0ABuffer2[j+3]=int_SP0ABuffer5[i+1];/*Copy ADC buffer to DAC buffer */
					   ...
					   int_SP0ABuffer2[j+6]=int_SP0ABuffer5[i];/*Copy ADC buffer to DAC buffer */
					   int_SP0ABuffer2[j+7]=int_SP0ABuffer5[i+1];/*Copy ADC buffer to DAC buffer */
				   }
				   CallbackCount=0;
				}
...
void SRU_Init(void)
{
	...
	*pREG_PADS0_DAI1_IE=0xfffff;
    ...
    SRU2(SPT4_AD1_O,DAI1_PB01_I); /* SPORT 4A to DAC*/
...
static void PrepareDescriptors (void)
{
	...
	iDESC_LIST_1_SP4A.XCount		= COUNT*4;
	...
	iDESC_LIST_2_SP4A.XCount		= COUNT*4;
...
static int Sport_Init()
{
	...
	eResult = adi_sport_DMATransfer(hSPORTDev4BRx,&iSRC_LIST_1_SP4B,(DMA_NUM_DESC),ADI_PDMA_DESCRIPTOR_LIST, ADI_SPORT_CHANNEL_SEC);
	CHECK_RESULT(eResult);
	/* Submit the first buffer for Tx.  */
	eResult = adi_sport_DMATransfer(hSPORTDev4ATx,&iDESC_LIST_1_SP4A,(DMA_NUM_DESC),ADI_PDMA_DESCRIPTOR_LIST, ADI_SPORT_CHANNEL_SEC);
	CHECK_RESULT(eResult);
